...
print('Cov_U1_U2'); print(Cov_U1_U2)
...
weighted_C_half_func<-function(inv_C_train,weight,dim_U1,dim_U2,tune_weight_method,C_half_train){
    if(weight == 0){
        return (C_half_train)
    }
    ...
    if(tune_weight_method == 1){
        ...
        inv_C_train[c(1:dim_U1),c(1:dim_U1)]<-inv_C_train[c(1:dim_U1),c(1:dim_U1)]*(1+weight)
        inv_C_train[c(1:dim_U1),-c(1:dim_U1)]<-inv_C_train[c(1:dim_U1),-c(1:dim_U1)]*sqrt(1+weight)
        inv_C_train[-c(1:dim_U1),c(1:dim_U1)]<-inv_C_train[-c(1:dim_U1),c(1:dim_U1)]*sqrt(1+weight)
...
htlgmm.default<-function(
    ...
    if(tune_weight){
        if(!tune_weight_method%in%c(1:6)){
            stop("Select weight tuning method from the following ones: 1, 2, 3, 4, 5, 6. ")
        }
    }
    if(is.null(dim(Z)[1])){
        ...
        if(length(index_nonzero) > 1){
            ...
            if(!is.null(alpha)){
                ...
                if(alpha == 10){
                    ...
                    return_list<-c(return_list,
                                   list("selected_vars1"=fC(C_half1,inv_C),
                                        "selected_vars2"=fC(C_half1,inv_C1)))
                }
            }
        }}
    if(is.null(fix_C) & is.null(fix_inv_C)){
